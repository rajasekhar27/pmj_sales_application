...
import {
  ...
  useGetCurrentTrendingProductsQuery,
...
export default function CurrentTrendsProducts() {
  const dispatch = useDispatch();
  const currentTrend = "current-trend-products";
  ...
  const { data: catalogueProducts } = useGetLimitCurrentTrendingQuery(
    {
      ...
      <>
        <div className="flex justify-between items-center  ">
          <div className="text-[18px] text-bg40 font-semibold">
            Current Trending Jewellery
          </div>
          <div className="">
            <button
              onClick={() =>
                router.push(
                  `/catalogue/product-listing/${router?.query?.id}/${currentTrend}`
                )
              }
              className="text-end flex items-center justify-center  rounded-md h-[33px] text-sm bg-bg40 text-white  ml-1 w-[92px] "
            >
              View All
            </button>
          </div>
